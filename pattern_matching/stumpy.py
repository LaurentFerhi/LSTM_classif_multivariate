...
def z_norm(a, axis=0, threshold=STUMPY_STDDEV_THRESHOLD):
    """
    ...
    threshold : float, default to STUMPY_STDDEV_THRESHOLD
...
def get_max_window_size(n):
    ...
    -------
    max_m : int
        The maximum window size allowed given `STUMPY_EXCL_ZONE_DENOM`
    """
    max_m = (
        int(
            n
            - np.floor(
                (n + (STUMPY_EXCL_ZONE_DENOM - 1))
                // (STUMPY_EXCL_ZONE_DENOM + 1)
            )
        )
        - 1
    )
...
def compute_mean_std(T, m):
    ...
    """
    num_chunks = STUMPY_MEAN_STD_NUM_CHUNKS
    max_iter = STUMPY_MEAN_STD_MAX_ITER
...
def _calculate_squared_distance(
    ...
    if np.isinf(M_T) or np.isinf(μ_Q):
        ...
        denom = (σ_Q * Σ_T) * m
        denom = max(denom, STUMPY_DENOM_THRESHOLD)
